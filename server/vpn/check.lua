FAC.LoadWhitelistedIPs = function()
    ...
    if (not whitelistedIPs) then
        ...
        FAC.WhitelistedIPs = whitelistedIPs
    end

    FAC.WhitelistedIPsLoaded = true
end

FAC.AddIPToWhitelist = function(ip)
    if (not FAC.ValidIP(ip)) then
        return false
    end

    local ipAlreadyAdded = false

    for _, _ip in pairs(FAC.WhitelistedIPs) do
        if (_ip == ip) then
            ipAlreadyAdded = true
        end
    end
    ...
    table.insert(FAC.WhitelistedIPs, ip)

    FAC.SaveWhitelistedIPs()

    return true
end

FAC.SaveWhitelistedIPs = function()
    SaveResourceFile(GetCurrentResourceName(), 'data/ignore-ips.json', json.encode(FAC.WhitelistedIPs or {}, { indent = true }), -1)
end

FAC.GetPlayerIP = function(playerId)
